...
if __name__ == '__main__':
    ...
    # audio = pydub.AudioSegment.from_file('dataset/audio/dog.wav').set_frame_rate(SAMPLING_RATE).set_channels(1)
    # audio = (np.fromstring(audio._data, dtype="int16") + 0.5) / (0x7FFF + 0.5)
    # # audio = (np.fromstring(audio._data, dtype="float32"))
    # audio_queue.append(audio.copy())
    # print(audio.shape)

    count = 0
    for block in sf.blocks('dataset/audio/bb.wav', blocksize=BLOCK_SIZE, dtype='float32'):
        if count < 100:
            # audio_queue.append(block[:, 0].copy())
            audio_queue.append(block.copy())
        count = count + 1
    ...
    while True:
        ...
        if len(processing_queue) > PROCESSING_DELAY + 1:  # +1 for JavaScript streaming delay
            ...
            # step_audio = step_audio[n_samples:1]
